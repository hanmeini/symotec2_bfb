...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($jabatan)) {
        echo "<script>alert('Bagian harus diisi');</script>";
    } else {
        if ($id_edit == 0) {
            $cek = $conn->prepare("SELECT id FROM bagian  WHERE nama_bagian = ?");
            $cek->bind_param("s", $jabatan);
            $cek->execute();
            $cek->store_result();
            if ($cek->num_rows > 0) {
                echo "<script>alert('Bagian sudah ada');</script>";
            } else {
            ...
            $stmt = $conn->prepare("UPDATE bagian SET nama_bagian = ? WHERE id = ?");
...
if (isset($_GET['edit'])) {
    ...
    $stmt = $conn->prepare("SELECT id, nama_bagian FROM bagian WHERE id = ?");
...
// Ambil semua data
$result = $conn->query("SELECT id, nama_bagian FROM bagian");
?>
...
<head>
    <meta charset="UTF-8">
    <title>Input dan Tampil Data Bagian</title>
...
<div class="container">
    <h1><?= $editData ? 'Edit' : '' ?> BAGIAN / DEPARTEMEN</h1>
    <form method="POST" class="form-input">
        <label for="jabatan">Input Bagian:</label>
        ...
        <?php if ($result->num_rows > 0): ?>
            <?php while ($row = $result->fetch_assoc()): ?>
                <tr>
                    ...
                    <td>
                        <a class="action-button" href="?edit=<?= $row['id'] ?>" onclick="return confirmEdit();">Edit</a>
                    </td>
                </tr>
            <?php endwhile; ?>
        <?php else: ?>
