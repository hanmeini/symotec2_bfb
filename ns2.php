...
<div style="margin-top: 15px;">
<a href="export_ns.php?bulan_awal=<?= urlencode($bulan_awal) ?>&bulan_akhir=<?= urlencode($bulan_akhir) ?>&tahun=<?= urlencode($tahun_terpilih) ?>&location=<?= urlencode($loc_terpilih) ?>&devisi=<?= urlencode($dev_terpilih) ?>" target="_blank">
...
$query = "
    ...
    FROM coa c
    LEFT JOIN jurnal j ON c.account_code = j.coa
     AND j.journal_number IS NOT NULL   -- ← FILTER BARU DITAMBAHKAN DI SINI
        $filter_location
        $filter_devisi
...
while ($stmt->fetch()) {
    ...
    if (in_array($first_digit, ['1', '2', '3'])) {
        if ($first_digit === '1') {
          $saldo_awal_debet = (float)$saldo_awal;

        } elseif (in_array($first_digit, ['2', '3'])) {
            $saldo_awal_kredit =(float)-$saldo_awal;
        }
    }
...
// ======================================================
// HITUNG SELISIH NERACA TAHUN SEBELUMNYA & TAHUN BERJALAN
// ======================================================

$tahun_awal = $tahun . '-01-01';

// Selisih Neraca Tahun Sebelumnya (< tahun terpilih)
$sql_prev = "
SELECT
    COALESCE(SUM(
        CASE
            WHEN c.account_code LIKE '1%' THEN (j.debet - j.kredit)
            ELSE 0
        END
    ),0) AS aset,
...
$stmt_prev->bind_param("s", $tahun_awal);
$stmt_prev->execute();
$res_prev = $stmt_prev->get_result()->fetch_assoc();

$selisih_tahun_sebelumnya =
    (float)$res_prev['aset']
    - (float)$res_prev['kewajiban_modal'];

$stmt_prev->close();


// Selisih Neraca Tahun Berjalan (= tahun terpilih)
$sql_curr = "
SELECT
    COALESCE(SUM(
        CASE
...
$stmt_curr->bind_param("s", $cutoff_prev);
$stmt_curr->execute();
...
if (isset($data['31.02.01.001'])) {
    ...
    $data['31.02.01.001']['debet_neraca'] = 0;
       $data['31.02.01.001']['saldo_awal_kredit'] = $selisih_tahun_sebelumnya;
    $data['31.02.01.001']['kredit_neraca'] = $selisih_tahun_sebelumnya;
    $data['31.02.01.001']['saldo_akhir'] = $selisih_tahun_sebelumnya;
}
...
foreach ($data as $key => &$values) {
    ...
    $values['saldo_awal_debet'] = 0;
    $values['saldo_awal_kredit'] = 0;
    $values['total_debet'] = 0;
    $values['total_kredit'] = 0;
    $values['saldo_akhir'] = 0;
    $values['lr_debet'] = 0;
    $values['lr_kredit'] = 0;
    ...
    foreach ($data as $child) {
        ...
        if ($match) {
            $values['saldo_awal_debet'] += $child['saldo_awal_debet'];
            $values['saldo_awal_kredit'] += $child['saldo_awal_kredit'];
            $values['total_debet'] += $child['total_debet'];
            $values['total_kredit'] += $child['total_kredit'];
            $values['saldo_akhir'] += $child['saldo_akhir'];
            $values['lr_debet'] += $child['lr_debet'];
            $values['lr_kredit'] += $child['lr_kredit'];
...
foreach ($data as $row) {
    ...
    echo "<tr class=\"$layer_class\"$highlight>
            ...
            <td align='right'>" . number_format($row['saldo_awal_debet'],0) . "</td>
            <td align='right'>" . number_format($row['saldo_awal_kredit'], 0) . "</td>
            <td align='right'>" . number_format($row['total_debet'], 0) . "</td>
            <td align='right'>" . number_format($row['total_kredit'], 0) . "</td>
            ...
            <td align='right'>" . number_format($row['debet_neraca'], 0) . "</td>
            <td align='right'>" . number_format($row['kredit_neraca'], 0) . "</td>
        </tr>";
}
...
echo "<tr style='font-weight:bold; background:#e0e0e0;'>
    ...
    <td align='right'>" . number_format($total['saldo_awal_debet'], 0) . "</td>
    <td align='right'>" . number_format($total['saldo_awal_kredit'], 0) . "</td>
    <td align='right'>" . number_format($total['total_debet'], 0) . "</td>
    <td align='right'>" . number_format($total['total_kredit'], 0) . "</td>
    <td align='right'>" . number_format($total['saldo_akhir'], 0) . "</td>
    <td align='right'>" . number_format($total['lr_debet'], 0) . "</td>
    <td align='right'>" . number_format($total['lr_kredit'], 0) . "</td>
    <td align='right'>" . number_format($total['debet_neraca'], 0) . "</td>
    <td align='right'>" . number_format($total['kredit_neraca'], 0) . "</td>
</tr>";
...
echo "<tr style='font-weight:bold; background:#d0ffd0;'>
    <td colspan='11' align='center'>LABA RUGI BULAN INI</td>
    <td colspan='1' align='right'>" . number_format($laba_rugi_bulan_ini, 0) . "</td>
</tr>";
// Baris Selisih Neraca
$selisih_neraca = $total['debet_neraca'] - $total['kredit_neraca'];
echo "<tr style='font-weight:bold; background:#ffd0d0;'>
    <td colspan='11' align='center'>SELISIH NERACA</td>
    <td colspan='2' align='right'>" . number_format($selisih_neraca, 0) . "</td>
</tr>";
...
<form id="formClose" method="POST">
    <input type="hidden" name="bulan" value="' . $bulan_akhir . '">
    <input type="hidden" name="tahun" value="' . $tahun . '">
    <input type="submit" value="Close Data">
</form>

<script>
document.getElementById("formClose").addEventListener("submit", function(e) {
    const bulan = "' . $bulan_akhir . '";
    const tahun = "' . $tahun . '";

    const konfirmasi1 = confirm("Apakah Anda yakin akan closing data untuk  bulan " + bulan + " tahun " + tahun + "?");
    if (!konfirmasi1) {
        e.preventDefault(); // Batal jika user klik Cancel di tahap pertama
        return;
    }

    const konfirmasi2 = confirm("Konfirmasi ulang! Data bulan " + bulan + " dan tahun " + tahun + " akan dikunci.\nApakah Anda benar-benar ingin melanjutkan?");
    if (!konfirmasi2) {
        e.preventDefault(); // Batal jika user klik Cancel di tahap kedua
    }
});
</script>
';
