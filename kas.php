...
<head>
    ...
    <title>kas out www.symotech.id</title>
  ...
  .formatted-input input[type="number"] {
    ...
    z-index: 2;
    pointer-events: all; /* Input tetap aktif */
  }
  ...
  .formatted-input span {
    ...
    left: 0;
    z-index: 1;
    padding: 2px 5px; /* Sesuaikan padding */
    box-sizing: border-box;
  }
...
<body>
    ...
    <button type="button" onclick="openCalPopup()" style="display: flex; align-items: center; gap: 5px;">
        ...
        <form id="formJurnal" action="save_jurnalkas.php" method="post" enctype="multipart/form-data">
...
<br><br>
        ...
        FROM coa 
        WHERE layer = 4 AND account_code LIKE '111%'";
...
<td id="total_debet_container">
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        if (debetInput) {
            var debetValue = debetInput.value.replace(/,/g, ''); // Hapus koma
        ...
        if (kreditInput) {
            var kreditValue = kreditInput.value.replace(/,/g, ''); // Hapus koma
    ...
    // Format dan tampilkan total debet
    var formattedTotalDebet = formatRibuan(totalDebet.toFixed(2));
    document.getElementById("total_debet").innerText = formattedTotalDebet;
    document.getElementById("hidden_total_debet").value = totalDebet.toFixed(2);

    // Format dan tampilkan total kredit
    var formattedTotalKredit = formatRibuan(totalKredit.toFixed(2));
    document.getElementById("total_kredit").innerText = formattedTotalKredit;
