...
$sql = "SELECT idn, no_cn_dn, kode_booking, dn, id_cust, tanggal, id_parent FROM cndn WHERE dn > 0 AND inv IS NULL";
...
<head>
    <meta charset="UTF-8">
    <title>Daftar DN BELUM DIGUNAKAN > 0</title>
...
<body class="container mt-4">
    ...
    <h3 class="mb-4">DAFTAR DN BELUM DIGUNAKAN</h3>

    <form method="GET" class="form-inline mb-3">
            <a href="inapdn.php" class="btn btn-info" style="margin-right: 10px;">
       <i class="fas fa-plus-circle"></i> Tambah DN AP</a>
        ...
        <a href="alldnap.php" class="btn btn-secondary ml-2">Reset</a>
        <a href="exportdn_excel.php?filter_customer=<?= urlencode($filter_customer) ?>" class="btn btn-success ml-2">
            <i class="fas fa-file-excel"></i> Export Excel
        </a>
         <a href="alldndone.php" class="btn btn-info" style="margin-left: 10px;">
        <i class="fas fa-list"></i> DN Yang Sudah Digunakan
    </a>
    </form>
    </form>
                ...
                <?php if (!empty($data)): ?>
                    <?php foreach ($data as $row): ?>
                        <tr>
                            ...
                            <td class="text-right"><?= number_format($row['dn'] ?? 0, 2) ?></td>
                            <td><?= htmlspecialchars($row['id_parent'] ?? '', ENT_QUOTES) ?></td>
                            <td><?= htmlspecialchars($row['customer'] ?? '', ENT_QUOTES) ?></td>
                        </tr>
                    <?php endforeach; ?>
                    <tr class="font-weight-bold bg-light">
                        <td colspan="4" class="text-center">TOTAL DN</td>
                        <td class="text-right"><?= number_format($total_dn, 2) ?></td>
                        <td colspan="2"></td>
                    </tr>
                <?php else: ?>
