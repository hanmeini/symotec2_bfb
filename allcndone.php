...
if ($conn->connect_error||$conn->connect_error) die("Koneksi DB gagal.");

$filter = trim($_GET['filter_customer'] ?? '');
$where = "inv IS NOT NULL AND cn > 0";
if ($filter !== '') {
    ...
    if (count($ids)>0) $where .= " AND id_cust IN (".implode(',',$ids).")";
    else $where .= " AND 0";
}

$sql = "SELECT idn, no_cn_dn, kode_booking, cn, tanggal, id_parent, id_cust, inv
        FROM cndn WHERE $where ORDER BY tanggal,idn";
$res = $conn->query($sql);

$total_cn = 0;
?>
<!DOCTYPE html><html lang="id"><head><meta charset="UTF-8">
<title>CN yang sudah digunakan</title>
<link rel="stylesheet"
...
<body>
    <div class="table-container">
        ...
        <a href="allcnap.php" class="left-icon">
            <i class="fa-solid fa-circle-left"></i>
        </a>
<h1>CN AP yang sudah digunakan</h1>
<form method="GET" style="text-align:center;" class="mb-3">
    ...
    <a href="allcndone.php" class="btn btn-secondary">
        <i class="fas fa-rotate-left"></i> Reset
    </a>

    <a href="exportcndone_excel.php?filter_customer=<?= urlencode($filter) ?>" class="btn btn-success">
        <i class="fas fa-file-excel"></i> Export Excel
    </a>
</form>
<table>
<thead><tr>
    <th>ID</th><th>No CN</th><th>Kode Booking</th><th>Tanggal</th><th>Nominal</th>
    <th>INV</th><th>Parent</th><th>Customer</th>
</tr></thead><tbody>
<?php if ($res && $res->num_rows>0): while($row=$res->fetch_assoc()):
    $cust = '(Tidak ditemukan)';
    $rc = $conn->query("SELECT customer FROM customer WHERE id=".intval($row['id_cust']));
...
<?php endwhile; ?>
<tr><td colspan="4"><strong>TOTAL CN</strong></td><td align="right"><?= number_format($total_cn,2) ?></td>
<td colspan="3"></td></tr>
<?php else: ?>
