...
if ($filter_customer !== '') {
    ...
    $sql_cust = "SELECT id FROM cust WHERE customer LIKE '%$filter_customer_esc%'";
...
<form method="GET" class="form-inline mb-3" style="text-align:center;">
    ...
    <a href="export_titipandone.php?filter_customer=<?= urlencode($filter_customer) ?>" class="btn btn-success">Export Excel</a>
</form>
...
if ($result_pph23 && $result_pph23->num_rows > 0) {
    ...
    while ($row = $result_pph23->fetch_assoc()) {
        ...
        $sql_customer = "SELECT nama AS customer FROM cust WHERE id = " . intval($row['cust_id']);
        ...
        $sql_used = "SELECT id, nominal FROM titipan WHERE id_parent = " . intval($row['id']) . " LIMIT 1";
