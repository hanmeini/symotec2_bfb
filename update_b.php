...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Update query
    $stmt = $conn->prepare("UPDATE b SET nama_b = ?, jenis = ?, brand = ?, harga_b = ?, ppn_b = ?, hargat_b = ?, dpp = ?, harga_m = ?, ppn_m = ?, hargat_m = ? WHERE id = ?");
    $stmt->bind_param("sssdddddddi", $nama_b, $jenis, $brand, $harga_b, $ppn_b, $hargat_b, $dpp, $harga_m, $ppn_m, $hargat_m, $id);
